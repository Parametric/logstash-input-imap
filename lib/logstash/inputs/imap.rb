    ...
    @codec.decode("Real message should replace this") do |event|
      ...
      mail.header_fields.each do |header|
        begin
          ...
          # Log and try to keep going
          # REVIEW: Do we need to make sure the critical headers get captured at least?
          @logger.warn("Issue parsing a header field: #{header.name.to_s}, #{mail.message_id}")
        end
      end
